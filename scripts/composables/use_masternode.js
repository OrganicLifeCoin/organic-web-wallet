...
import { sanitizeMasternodeRecords } from '../deterministic_masternode_records.js';
...
export const useMasternode = defineStore('masternode', () => {
    ...
    const deterministicMasternodes = ref([]);
    watch(
        localProposals,
    ...
    watch(
        ...
        async (records) => {
            ...
            if (account) {
                account.deterministicMasternodes =
                    sanitizeMasternodeRecords(toRaw(records));
                await database.updateAccount(account, true);
            }
        },
        { deep: true }
    ...
    const fetchDeterministicMasternodesFromDatabase = async () => {
        const database = await Database.getInstance();
        const account = await database.getAccount(
            wallets.activeWallet.getKeyToExport()
        );
        deterministicMasternodes.value = sanitizeMasternodeRecords(
            account?.deterministicMasternodes
        );
    };
    ...
    getEventEmitter().on('wallet-import', () => {
        ...
        fetchDeterministicMasternodesFromDatabase().then(() => {});
    });
    ...
    getEventEmitter().on('wallet-selected', () => {
        fetchDeterministicMasternodesFromDatabase().then(() => {});
    });
    return {
        masternodes,
        localProposals,
        deterministicMasternodes,
        fetchDeterministicMasternodesFromDatabase,
    };
});
