...
export class Database {
    ...
    async addAccount(account) {
        ...
        if (!(account instanceof Account)) {
            ...
            createAlert(
                'warning',
                '<b>Account Creation Error</b><br>Logs were dumped in your Browser Console<br>Please submit these privately to the OrganicLifeCoin Team!'
            );
            throw new Error(
     ...
     *
     * This will also apply new Account keys from OLC Wallet updates automatically, and check high-level type safety.
     *
     * ---
     *
    ...
    async updateAccount(account, allowDeletion = false) {
        ...
        if (!(account instanceof Account)) {
            ...
            createAlert(
                'warning',
                '<b>DB Update Error</b><br>Your wallet is safe, logs were dumped in your Browser Console<br>Please submit these privately to the OrganicLifeCoin Team!'
            );
            throw new Error(
                'addAccount was called with with an invalid account'
            );
        ...
        // If none exists; we should throw an error, as there's no reason for OLC Wallet to call `updateAccount` before an account was added using `addAccount`
        ...
        if (!cDBAccount) {
            ...
            createAlert(
                'warning',
                '<b>DB Update Error</b><br>Logs were dumped in your Browser Console<br>Please submit these privately to the OrganicLifeCoin Team!'
            );
            throw new Error(
     ...
     *
     * This also will apply new keys from OLC Wallet updates automatically, and check high-level type safety.
    ...
    static async create(name) {
        ...
        const db = await openDB(`MPW-${name}`, Database.version, {
            ...
            blocking: () => {
                ...
                // (This would only happen if the user opened another window after OLC Wallet got an update)
                database.close();
