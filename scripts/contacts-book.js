    ...
    if (!fPrompt) {
        ...
        strHTML += `
            <div class="shadowInnerCard" style="font-family: 'Montserrat'; text-align: start; margin: 0px 15px; border-radius: 10px; border: 1px solid #1d265a; background-color: #161a45; padding: 13px 13px; margin-bottom: 28px;">
                ...
                <div class="row">
                    <div class="col-6">
                        <button class="olc-button-small-cancel" style="height: 49px; width: 49px; padding-left: 12px;" onclick="OLCWallet.guiAddContactQRPrompt()">
                    ...
                    <div class="col-6 text-right">
                        <button class="olc-button-small" style="height: 42px; width: 97px;" onclick="OLCWallet.guiAddContact()">
        ...
        for (const cContact of account.contacts || []) {
            ...
            <div class="d-flex px-3 py-3 contactItem" style="padding-left: 10px !important; border-bottom: 1px solid #1d265a;">
                <div>
                    <img onclick="OLCWallet.guiAddContactImage('${i}')" class="ptr" style="margin-right:20px; width: 50px; height: 50px; border-radius: 100%; background-color: white; border: 2px solid #7f91d6;" ${
                cContact.icon ? 'src="' + cContact.icon + '"' : ''
            }>
                </div>
                <div style="width: 100%; line-height: 15px; margin-top: -6px;">
                    <span id="contactsName${i}" onclick="OLCWallet.guiEditContactNamePrompt('${i}')" style="word-wrap: anywhere; cursor:pointer; color: #DEDEE0; font-weight: 500; margin-top: 8px; display: block;">${sanitizeHTML(
                cContact.label
            )}</span>
                ...
                <div style="display: flex; justify-content: flex-end; align-items: center; padding-right: 6px; padding-left: 15px;">
                    <i style="cursor:pointer;" class="pIconBin" onclick="OLCWallet.guiRemoveContact(${i})">${pIconBin}</i>
                </div>
            </div>
            `;
            i++;
        ...
        strHTML += `
            <div class="text-right" style="margin-top: 15px; margin-bottom: 15px;">
                <button class="olc-button-small-cancel" style="height: 42px; width: 97px;"><span class="buttoni-text" id="contactsSelector-1"> Close </span></button>
            </div>
        `;
...
async function renderContactModal() {
    ...
    if (cAccount?.name) {
        ...
        // Render Copy Button
        doms.domModalQrLabel.innerHTML = `${translation.shareContactURL}<i onclick="OLCWallet.localContactToClipboard(event)" id="guiAddressCopy" class="pColor" style="position: absolute; right: 27px; margin-top: -1px; cursor: pointer; width: 20px;">${pIconCopy}</i>`;
        ...
        doms.domModalQrLabel.innerHTML =
            strSharedKey +
            `<i onclick="OLCWallet.toClipboard('${strSharedKey}', this)" id="guiAddressCopy" class="pColor" style="position: absolute; right: 27px; margin-top: -1px; cursor: pointer; width: 20px;">
                ${pIconCopy}
            </i>`;
        ...
        if (await hasEncryptedWallet()) {
            doms.domModalQR.innerHTML = `
                ...
                <button class="olc-button-small-cancel" style="height: 42px; width: 167px;" onclick="OLCWallet.guiSetAccountName('setContactName')"><span class="buttoni-text">${translation.createContact}</span></button>`;
        } else {
            doms.domModalQR.innerHTML = `
                    <center>
...
function renderAddress(strAddress) {
    ...
    doms.domModalQrLabel.innerHTML =
        ...
        `<i onclick="OLCWallet.toClipboard('${strAddress}', this)" id="guiAddressCopy" class="pColor" style="position: absolute; ${
    ...
    if (cWallet.isHD && !fXPub) {
        ...
        doms.domModalQrLabel.innerHTML += `<i onclick="OLCWallet.getNewAddress({ updateGUI: true, verify: true, shield: ${!cWallet.publicMode} })" class="pColor fa-solid fa-arrows-rotate fa-lg" style="position: absolute; right: 27px; margin-top: 10px; cursor: pointer; width: 20px;"></i>`;
    }
}
...
export async function guiAddContactPrompt(
    ...
    // Verify the address
    if (!isValidOLCAddress(strPubkey))
        return createAlert(
            'warning',
