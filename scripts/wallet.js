...
import { resetShieldToOLCStart } from './olc_shield.js';
...
export class Wallet {
    ...
    async #checkShieldSaplingRoot(networkSaplingRoot) {
        ...
        if (saplingRoot !== networkSaplingRoot) {
            ...
            // Also clear the corrupted shield backup from the account, so a
            // refresh cannot reload the broken tree again.
            const cAccount = await db.getAccount(this.getKeyToExport());
            if (cAccount && cAccount.shieldData) {
                cAccount.shieldData = '';
                await db.updateAccount(cAccount);
            }
    ...
    async #loadShieldFromDisk() {
        ...
        let loadRes;
        try {
            loadRes = await PIVXShield.load(cAccount.shieldData);
        } catch (e) {
            // Corrupted/incompatible shield backup: clear it so it does not
            // break wallet loading (or the sapling root check) again.
            debugLog(
                DebugTopics.WALLET,
                `Failed to parse shield backup, clearing it: ${e.message}`
            );
            cAccount.shieldData = '';
            await cDB.updateAccount(cAccount);
            this.#shield = null;
