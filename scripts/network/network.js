 ...
 * @property {string} name - OrganicLifeCoin address string
 ...
 * @property {number} decimals - Decimal places in the amounts (OrganicLifeCoin has 8 decimals)
...
export class Network {
    ...
    async getMasternodeStatus() {
        throw new Error('getMasternodeStatus must be implemented');
    }

    async generateMasternodeOperatorKey() {
        throw new Error('generateMasternodeOperatorKey must be implemented');
    }

    async prepareMasternode(_opts) {
        throw new Error('prepareMasternode must be implemented');
    }

    async submitMasternode(_opts) {
        throw new Error('submitMasternode must be implemented');
    }
...
export class RPCNodeNetwork extends Network {
    ...
    async #callRpcPost(api, body) {
        ...
        if (!cRes.ok) {
            ...
            try {
                errMessage = JSON.parse(errText).error || errText;
            } catch (_) {
                /* keep raw text */
            }
            throw new Error(errMessage || 'Failed to call rpc');
        }
    ...
    /**
     * Get the operator status of the masternode on the node
     * @returns {Promise<Object>} The masternode status
     */
    async getMasternodeStatus() {
        return await this.#callRPC('/getmasternodestatus');
    }

    async generateMasternodeOperatorKey() {
        return await this.#callRpcPost('/masternode/operator-key', {});
    }

    async prepareMasternode(opts) {
        return await this.#callRpcPost('/masternode/prepare', opts);
    }

    async submitMasternode(opts) {
        return await this.#callRpcPost('/masternode/submit', opts);
    }
...
export class ExplorerNetwork extends Network {
     ...
     * @param {string} addr - a OrganicLifeCoin address or xpub
    ...
    async #getPage(nStartHeight, addr, n, pageSize) {
        if (!(isXPub(addr) || isStandardAddress(addr))) {
            throw new Error('must provide either a OrganicLifeCoin address or a xpub');
        }
