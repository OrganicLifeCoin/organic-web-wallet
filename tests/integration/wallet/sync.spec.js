...
describe('Wallet sync tests', () => {
    ...
    it('recognizes immature balance', async () => {
        ...
        await mineBlocks(2);
        expect(watchOnlyWallet.immatureBalance - initImmatureBalance).toBe(
            superblockProfit
        );
        expect(watchOnlyWallet.balance - initBalance).toBe(0);
        expect(watchOnlyWallet.coldBalance - initialColdBalance).toBe(0);
        ...
        expect(watchOnlyWallet.balance - initBalance).toBe(superblockProfit);
        expect(watchOnlyWallet.coldBalance - initialColdBalance).toBe(0);
        ...
        await mineBlocks(1);
        expect(watchOnlyWallet.immatureBalance - initImmatureBalance).toBe(0);
        expect(watchOnlyWallet.balance - initBalance).toBe(superblockProfit);
        expect(watchOnlyWallet.coldBalance - initialColdBalance).toBe(
            coldStakeProfit
        );
    });
