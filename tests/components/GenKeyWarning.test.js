...
describe('GenKeyWarning tests', () => {
    ...
    it('GenKeyWarning (no encrypt)', async () => {
        ...
        await submitBtn.trigger('click');
        await nextTick();
        checkEventsEmitted(wrapper, 0, 1, 0);
    ...
    it('GenKeyWarning (with encrypt)', async () => {
        ...
        // A successful parent save hides the modal and clears its secrets.
        await wrapper.setProps({ showModal: false });
        await nextTick();
        await wrapper.setProps({ showModal: true });
        await nextTick();
        expect(
            wrapper.findComponent(Modal).find('[data-testid=newPasswordModal]')
                .element.value
        ).toBe('');
        expect(
            wrapper
                .findComponent(Modal)
                .find('[data-testid=currentPasswordModal]').element.value
        ).toBe('');
